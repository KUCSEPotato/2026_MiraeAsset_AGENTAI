...
> 후속 원격 동기화: `origin/main`의 `85dbe3a`를 가져와 이 브랜치와 통합했다. 아래 실제 QA 및 711개 회귀 검사 결과는 병합 전 `397b996` 수정에 대한 기록이다. 현재 QA 서버도 그 실행 버전을 유지한다. 병합 후 검증은 문서 마지막에 별도로 기록한다.
...
사용자의 pull·push 요청에 따라 최초 수정 커밋 `397b996`을 보존하고, 원격 `main`의 `85dbe3a9703c509e23959bd12a81531b7845ff3b`를 통합했다.

- 6개 파일의 충돌을 해결했다. 원격의 분류·수식어 처리, 관계 타입 허용 목록, 오류 사유 로그를 이번 기본값·최대 1회 복구 로직과 함께 유지했다.
- 관계 타입의 정규화·후보 schema·검증기가 동일한 허용 목록을 사용한다.
- 원문에 명시된 제외 표현은 규칙의 불완전한 긍정 필터를 보정할 수 있다. 제외 표현이 없는 필터를 부정하는 후보는 거부한다.
- 미지원 숫자 필터를 일반 의미 검색어로 바꾸는 후보는 거부한다. 주관적 필수 선정 조건도 계속 유지한다.
- 원문의 유일한 문자열 위치를 보정하는 정책에 맞춰, 오류 로그 검사는 원문에 실제로 없는 문자열을 사용하도록 fixture를 수정했다. 오류 사유 전파 검증은 유지했다.
- 병합 후 focused 검사: **420 passed**.
- 병합 후 전체 tracked Python 테스트 44개 파일: **836 passed, 109 skipped, 1 warning**, 78.96초. 생략은 PostgreSQL 테스트 환경 108개와 Next.js export 산출물 미제공 1개다.
- 최종 전체 회귀 출력: `/private/tmp/mirae-pull-merge-regression-final.txt`.
