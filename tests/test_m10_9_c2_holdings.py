...
from app.domain.models import ResolvedQuery, RetrievalSource
...
def test_canonical_schema_has_security_and_temporal_holding_grains() -> None:
    assert CANONICAL_V2_SCHEMA_VERSION == "m10.9-c2-canonical-v2"
...
def test_company_holding_queries_use_the_same_two_allow_listed_paths(
    name: str, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setenv("TRUSTED_HOLDINGS_RUNTIME_ENABLED", "1")
    parsed, grounded, plan = asyncio.run(_plan(f"{name}를 보유한 ETF"))
    ...
    assert paths[0]["target_values"] == [name]
    assert paths[1]["target_values"] == [None, name]
...
def test_graph_compiler_accepts_only_the_reviewed_holdings_path(
    ...
    _, _, plan = asyncio.run(_plan("삼성전자를 보유한 ETF"))
    step = next(item for item in plan.steps if item.source is RetrievalSource.GRAPH)
    compiled = compiler.compile(step, step.inputs["paths"][1], candidate_ids=["etf:1"])
    ...
    assert compiled.parameters["target_value_1"] == "삼성전자"
...
def test_holdings_query_is_fail_closed_before_runtime_activation(
    ...
    assert grounded.semantic_constraints[0].unsupported_reason is not None
