"""Build and activate the reviewed TIGER READY scope."""
...
from app.data.v2_schema import canonical_facts, entity_relations, fact_evidence_links  # noqa: E402
from app.external_data.holdings.kodex_scope import load_trusted_scope  # noqa: E402
from app.external_data.holdings.provider_contracts import TIGER_CONTRACT  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Activate the reviewed TIGER READY scope")
    parser.add_argument("--snapshot-root", type=Path, required=True)
    parser.add_argument("--kodex-snapshot-root", type=Path, required=True)
    ...
    reviewed = frozenset(
        row.constituent_ticker for row in kodex.holdings
        if row.constituent_ticker is not None
    )
    manifest_path, manifest = build_tiger_ready_scope(
        args.snapshot_root, reviewed_security_tickers=reviewed,
    )
